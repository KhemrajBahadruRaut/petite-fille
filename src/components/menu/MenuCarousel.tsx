...
import { subscribeToMenuCarouselRefresh } from "@/utils/menuCarouselRefresh";
import {
    apiUrl,
...
export default function MenuCarousel() {
    ...
    const fetchSlides = useCallback(async (signal: AbortSignal) => {
        try {
            ...
            if (!response.ok || !data.success) {
                throw new Error(data.message || "Failed to fetch menu carousel");
            }
    ...
    useLiveRefresh(fetchSlides, 1000);

    useEffect(() => {
        let refreshController: AbortController | null = null;
        const unsubscribe = subscribeToMenuCarouselRefresh(() => {
            refreshController?.abort();
            refreshController = new AbortController();
            void fetchSlides(refreshController.signal);
        });
