  ...
  return (
    ...
    >
      <div className="relative aspect-square overflow-hidden rounded-lg mb-4 shadow-md bg-gray-200">
        ...
        >
          <Heart className={`w-4 h-4 transition-all duration-300 ${isItemFavorite ? "text-red-500 fill-red-500" : "text-gray-600 hover:text-red-500"}`} />
        </button>
...
const MenuSection: React.FC<{ title: string; items: MenuItem[] }> = ({ title, items }) => {
  ...
  return (
    <section ref={sectionRef} className="py-12 px-6 max-w-7xl mx-auto">
      <h2 className={`text-3xl font-light text-gray-800 mb-8 opacity-0 -translate-x-5 ${isVisible ? "animate-fade-in-left" : ""}`}>
        {title}
      </h2>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 lg:gap-8">
        {items.map((item, index) => <MenuItemCard key={item.id} item={item} index={index} />)}
      </div>
    </section>
  );
};
