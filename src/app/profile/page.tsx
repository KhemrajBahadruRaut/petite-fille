...
const DELIVERY_STEPS: UserOrder["status"][] = [
  "paid",
  "processing",
  "shipped",
  "completed",
];
const PICKUP_STEPS: UserOrder["status"][] = [
  "paid",
  "processing",
  "ready_for_pickup",
  "completed",
...
const getGiftCardSenderDisplay = (
  card: Pick<UserGiftCard, "sender_name" | "sender_email">,
  fallback = "Sender",
) => {
  const senderName = card.sender_name?.trim();
  const senderEmail = card.sender_email?.trim();
...
export default function ProfilePage() {
  ...
  const fetchProfileData = useCallback(async () => {
    ...
    try {
      const [resResponse, gcResponse, ordersResponse] = await Promise.all([
        ...
        fetch(apiUrl("orders/get_orders.php"), {
          headers: authHeaders,
          cache: "no-store",
        }),
      ]);
      ...
      if (!resResponse.ok || !resPayload?.success) {
        setReservationsError(
          resPayload?.message || "Failed to load reservations.",
        );
      } else {
        setReservations(
          Array.isArray(resPayload?.reservations)
            ? resPayload.reservations
            : [],
        );
      }
  ...
  const fetchOrdersOnly = useCallback(async () => {
    if (!token) return;
    try {
      const res = await fetch(apiUrl("orders/get_orders.php"), {
        headers: authHeaders,
        cache: "no-store",
      });
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-20">
      <div className="mx-auto max-w-6xl space-y-6">
            ...
            <>
              ...
              <div className="pointer-events-none fixed inset-x-0 bottom-6 z-50 flex justify-center px-4">
                ...
                >
                  ...
                  <div
                    className={`h-1 w-full ${toast.type === "success" ? "bg-emerald-500" : toast.type === "error" ? "bg-rose-500" : "bg-amber-500"}`}
                  />
                </motion.div>
              </div>
            </>
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          {!!ordersError && (
            <p className="mt-2 text-xs text-red-600">{ordersError}</p>
          )}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      >
                        ...
                        <span className="font-medium text-gray-700">
                          ${Number(order.total_amount).toFixed(2)} AUD
                        </span>
                      </div>
                    </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
            {giftCards.received.map((card) => (
              ...
              >
                <div className="relative aspect-[1720/635] w-full overflow-hidden rounded-lg bg-white">
                  <img
                  ...
                  <div className="absolute left-[48.5%] top-[61%] w-[48%] break-words text-[clamp(9px,1.2vw,12px)] leading-tight text-[#8b7b67]">
                    {getGiftCardSenderDisplay(card)}
                  </div>

                  <div className="absolute left-[48.5%] top-[76%] w-[48%] break-words font-mono text-[clamp(8px,1vw,13px)] leading-tight tracking-wide text-[#8b7b67]">
                    {card.code}
                  </div>
                </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
            {giftCards.sent.map((card) => (
              <div
                key={card.id}
                className="rounded-xl border-2 border-blue-100 bg-blue-50 p-3"
              >
                <div className="relative aspect-[1720/635] w-full overflow-hidden rounded-lg bg-white">
                  <img
                    src="/giftcard/gift-card-template.png"
                  ...
                  <div className="absolute left-[48.5%] top-[61%] w-[48%] break-words text-[clamp(9px,1.2vw,12px)] leading-tight text-[#8b7b67]">
                    {getGiftCardSenderDisplay(card, user.name)}
                  </div>
